    ...
    <el-card>
      ...
      :expand-type="false" show-index index-text="#" border>
        ...
        <template slot="isok" slot-scope="scope">
          <i class="el-icon-success" style="color: lightblue"
          v-if="scope.row.cat_deleted === false"></i>
          <i class="el-icon-error"
          style="color: red" v-if="scope.row.cat_deleted === true" ></i>
        </template>
...
<script>
  export default {
    ...
    methods:{
      ...
      // 监听 handleCurrentChange 改变事件
      handleCurrentChange(newPage) {
        this.queryInfo.pagenum = newPage
        this.getCateList()
      },
      // 点击按钮，展示添加分类的对话框 改变事件
      showAddCateDialog(){
        // 先获取父级分类的数据列表
      ...
      parentCateChanged() {
        ...
        // 如果 selectedKeys 数组中的 length 大于 0 证明选中的是父级分类
        // 反之，就说明没有选中任何父级分类
